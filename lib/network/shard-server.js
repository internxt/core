...
ShardServer.prototype.routeConsignment = function(req, res) {
  ...
  this.isAuthorized(token, hash, (err, contact) => {
    if (err) {
      return res.status(401).send({ result: err.message });
    }
    ...
    this._manager.load(hash, function(err, item) {
      if (err) {
        return res.status(404).send({ result: err.message });
      }

      const contract = item.getContract(contact);
      if (!contract) {
        return res.status(500).send({ result: 'Unable to locate contract'});
      }
      const bridgeExtendedKey = contract.get('renter_hd_key');
      const bridge = self.farmerInterface.bridges.get(bridgeExtendedKey);
      if (!bridge) {
        return res.status(500).send({ result: 'Not connected to bridge'});
      }

      const shardsize = contract.get('data_size');
      if (!Number.isInteger(shardsize)) {
        return res.status(500).send({ result: 'Data size is not an integer' });
      }
      ...
      // eslint-disable-next-line max-statements
      (self.farmerInterface && self.farmerInterface._checkSpaceAvailable ?
        self.farmerInterface._checkSpaceAvailable :
        function(p, cb) { process.nextTick(() => cb(null, { available: Number.MAX_SAFE_INTEGER })); }
      )(storagePath, (err, info) => {
        if (err) {
          return res.status(503).send({ result: 'Unable to determine free space'} );
        }

        if (info.available - shardsize <= constants.FREE_SPACE_PADDING) {
          self._log.warn('disk space is at maximum capacity');
          return res.status(503).send({ result: 'No space left'});
        }
        ...
        if (typeof item.shard.write !== 'function') {
          ...
          return res.status(304).send({ result: 'Consignment completed' });
        }
        ...
        req.on('end', function() {
          ...
          res.status(200).send({ result: 'Consignment completed' });
