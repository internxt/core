...
/**
 * Default hook to check available disk space. Can be stubbed in tests or
 * overridden by higher-level network objects. By default it reports a large
 * amount of available space so that removing the native `diskusage` dependency
 * does not change runtime behavior.
 */
FarmerInterface.prototype._checkSpaceAvailable = function(storagePath, callback) {
  process.nextTick(() => callback(null, { available: Number.MAX_SAFE_INTEGER }));
}
...
FarmerInterface.Negotiator = function (contract, callback) {
  ...
  (this._checkSpaceAvailable || function(p, cb) { process.nextTick(() => cb(null, { available: Number.MAX_SAFE_INTEGER })); })(this._options.storagePath, (err, info) => {
    if (err) {
